...
async function setupTestDatabase() {
  ...
  try {
    ...
    if (userCheck.length === 0) {
      ...
      await adminDb.unsafe(`CREATE USER ${APP_USER} WITH PASSWORD '${APP_PASSWORD}'`)
    }
  } finally {
    adminDb.close()
  }
  ...
  const testDbAdminUrl = ADMIN_DATABASE_URL.replace(/\/[^/]+$/, `/${TEST_DB_NAME}`)
  ...
  try {
    ...
    await testDbAdmin.unsafe(`GRANT ALL ON ALL TABLES IN SCHEMA public TO ${APP_USER}`)
    await testDbAdmin.unsafe(`GRANT ALL ON ALL SEQUENCES IN SCHEMA public TO ${APP_USER}`)
    await testDbAdmin.unsafe(`ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES TO ${APP_USER}`)
    await testDbAdmin.unsafe(`ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON SEQUENCES TO ${APP_USER}`)
  } finally {
    testDbAdmin.close()
  }
}
