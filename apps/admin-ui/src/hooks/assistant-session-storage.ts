/**
 * Pure utility functions for assistant session history persistence.
 *
 * This module is intentionally separated from `use-assistant-session-history.ts`
 * so that lightweight consumers (e.g. the AssistantLauncher button) can check
 * session state without pulling in `@assistant-ui/react` and its dependencies.
 */
 ...
 */
export function convertPartToUIFormat(part: Record<string, unknown>): unknown {
  if (part.type === "text") {
 ...
 */
export function toStorableUIMessage(msg: unknown): UIMessage | null {
