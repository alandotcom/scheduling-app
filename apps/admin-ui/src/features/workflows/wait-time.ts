...
export function formatTriggerTime(
  date: Date,
  timeZone?: string,
): TriggerTimeDisplay {
  const formatter = new Intl.DateTimeFormat("en-US", {
  ...
  const timezoneName =
    typeof parts.timeZoneName === "string" ? parts.timeZoneName : undefined;

  return {
    main: `${parts.month} ${parts.day}, ${parts.year} ${parts.hour}:${parts.minute}${dayPeriod}`,
    timezone: timezoneName,
  };
}

export function hasDynamicExpression(value: unknown): boolean {
  if (typeof value !== "string") {
    return false;
  }
  ...
  return (
    trimmed.includes("{{") ||
    trimmed.includes("@") ||
    /\b[A-Za-z_][A-Za-z0-9_]*\.[A-Za-z_][A-Za-z0-9_.]*/.test(trimmed)
  );
}
