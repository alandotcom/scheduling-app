...
import { createJourneyActionSendSlackExecuteFunction } from "./journey-action-send-slack-execute.js";

describe("journey action send slack execute function", () => {
  test("configures cancelOn for delivery cancellation events", () => {
    const fn = createJourneyActionSendSlackExecuteFunction(async () => ({
  ...
  test("forwards event payload to delivery worker", async () => {
    ...
    const fn = createJourneyActionSendSlackExecuteFunction(executeDelivery);
  ...
  test("uses the injected slack dispatcher dependency", async () => {
    const dispatchDelivery = mock(
      async (_input: JourneyDeliveryDispatchInput) => ({
        providerMessageId: "slack-provider-message-1",
      }),
    );
    ...
    const fn = createJourneyActionSendSlackExecuteFunction(
      executeDelivery,
      dispatchDelivery,
    );
    const t = new InngestTestEngine({ function: fn });
