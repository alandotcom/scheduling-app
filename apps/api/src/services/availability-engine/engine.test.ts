...
describe('AvailabilityEngine', () => {
  ...
  beforeEach(async () => {
    ...
    const [at] = await db.insert(appointmentTypes).values({
      orgId: org.id,
      name: 'Consultation',
      durationMin: 60,
      paddingBeforeMin: 0,
    ...
    engine = new AvailabilityEngine(db as any)
  })
  ...
  describe('getAvailableSlots', () => {
    ...
    test('respects max per day limit', async () => {
      ...
      const [highCapAt] = await db.insert(appointmentTypes).values({
        orgId: org.id,
        name: 'Group Session',
        durationMin: 60,
        capacity: 10,
    ...
    test('returns empty for unlinked calendars', async () => {
      ...
      const [cal2] = await db.insert(calendars).values({
        orgId: org.id,
        name: 'Unlinked Room',
        timezone: 'America/New_York',
      }).returning()
  ...
  describe('capacity handling', () => {
    test('allows multiple bookings up to capacity', async () => {
      ...
      const [groupAt] = await db.insert(appointmentTypes).values({
        orgId: org.id,
        name: 'Group Class',
        durationMin: 60,
        capacity: 3,
  ...
  describe('padding handling', () => {
    test('accounts for padding when checking overlaps', async () => {
      ...
      const [paddedAt] = await db.insert(appointmentTypes).values({
        orgId: org.id,
