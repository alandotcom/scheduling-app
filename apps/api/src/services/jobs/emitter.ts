...
export async function emitEvent<T>(
  ...
  const insertFn = async (database: Database) => {
...
export const events = {
  // Appointment events
  appointmentCreated: <T>(orgId: string, payload: T, tx?: Database) =>
    emitEvent(orgId, 'appointment.created', payload, tx),

  appointmentUpdated: <T>(orgId: string, payload: T, tx?: Database) =>
    emitEvent(orgId, 'appointment.updated', payload, tx),

  appointmentCancelled: <T>(orgId: string, payload: T, tx?: Database) =>
    emitEvent(orgId, 'appointment.cancelled', payload, tx),

  appointmentRescheduled: <T>(orgId: string, payload: T, tx?: Database) =>
    emitEvent(orgId, 'appointment.rescheduled', payload, tx),

  appointmentNoShow: <T>(orgId: string, payload: T, tx?: Database) =>
    emitEvent(orgId, 'appointment.no_show', payload, tx),

  // Calendar events
  calendarCreated: <T>(orgId: string, payload: T, tx?: Database) =>
    emitEvent(orgId, 'calendar.created', payload, tx),

  calendarUpdated: <T>(orgId: string, payload: T, tx?: Database) =>
    emitEvent(orgId, 'calendar.updated', payload, tx),

  calendarDeleted: <T>(orgId: string, payload: T, tx?: Database) =>
    emitEvent(orgId, 'calendar.deleted', payload, tx),

  // Appointment type events
  appointmentTypeCreated: <T>(orgId: string, payload: T, tx?: Database) =>
    emitEvent(orgId, 'appointment_type.created', payload, tx),

  appointmentTypeUpdated: <T>(orgId: string, payload: T, tx?: Database) =>
    emitEvent(orgId, 'appointment_type.updated', payload, tx),

  appointmentTypeDeleted: <T>(orgId: string, payload: T, tx?: Database) =>
    emitEvent(orgId, 'appointment_type.deleted', payload, tx),

  // Resource events
  resourceCreated: <T>(orgId: string, payload: T, tx?: Database) =>
    emitEvent(orgId, 'resource.created', payload, tx),

  resourceUpdated: <T>(orgId: string, payload: T, tx?: Database) =>
    emitEvent(orgId, 'resource.updated', payload, tx),

  resourceDeleted: <T>(orgId: string, payload: T, tx?: Database) =>
    emitEvent(orgId, 'resource.deleted', payload, tx),

  // Location events
  locationCreated: <T>(orgId: string, payload: T, tx?: Database) =>
    emitEvent(orgId, 'location.created', payload, tx),

  locationUpdated: <T>(orgId: string, payload: T, tx?: Database) =>
