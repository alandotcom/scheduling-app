...
## Design-critic reconciliation checks (R31/R40/R44/R45)

- Journey run routes currently expose run read endpoints (`/journeys/{id}/runs`, `/journeys/runs/{runId}`) but no explicit run-cancel endpoint, while service cancellation behavior is currently scoped to journey-level active-run cancellation (`apps/api/src/routes/journeys.ts:141`, `apps/api/src/routes/journeys.ts:163`, `apps/api/src/services/journeys.ts:569`, `apps/api/src/services/journeys.ts:899`, `apps/api/src/services/journeys.ts:1083`).
- Wait execution currently follows configured schedule with no mode-specific acceleration branch: planner computes scheduled timestamps from wait nodes and worker sleeps until due time via runtime/Inngest sleep (`apps/api/src/services/journey-planner.ts:381`, `apps/api/src/services/journey-planner.ts:395`, `apps/api/src/services/journey-delivery-worker.ts:350`, `apps/api/src/services/journey-delivery-worker.ts:352`, `apps/api/src/inngest/functions/journey-delivery-scheduled.ts:29`).
- Delivery idempotency is run-scoped by construction: deterministic delivery keys include `journeyRunId`, and DB uniqueness enforces `(org_id, deterministic_key)`; run identity itself is version/appointment/mode scoped, which leaves journeys independent rather than globally deduped (`apps/api/src/services/journey-planner.ts:307`, `apps/api/src/services/journey-planner.ts:312`, `packages/db/src/schema/index.ts:570`, `packages/db/src/schema/index.ts:614`).
- Logger behavior is not yet encoded as an execution adapter path: planner only materializes deliveries for `send-message`, adapters currently support only `email|slack`, and run timelines render persisted deliveries (including `logger` rows when present) (`apps/api/src/services/journey-planner.ts:390`, `apps/api/src/services/journey-delivery-adapters.ts:26`, `apps/api/src/services/journey-delivery-adapters.ts:43`, `apps/api/src/services/journeys.ts:865`, `apps/api/src/services/journeys.test.ts:727`).
